...
// Add a cross reference to decision-making in the previous line.
...
* Right positions: <<organizational_structure>>
* Right numbers: <<staff_in_adequate_numbers_for_your_expectations>>
...
As a reminder, <<expertise,expertise>> is one of the components of <<individual_performance,individual performance>>.
...
The structure should be aligned with the expertise component of the relevant <<define_role_profiles,role profiles>>.

===== Populate the map

The most effective and efficient approach to populating the map is _self-assessment_. Send out a form for/access to the spreadsheet and have people self-rate their expertise from [0, 4] (as per  <<adopt_our_individual_performance_rating_template, individual performance rating>>).
...
* The results can be used as input into <<strategize_personal_development,personal development strategies>>.
