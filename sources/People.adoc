...
The only possible reason that a person can have for defending the idea that high performance is not a fundamental requirement for senior positions, is to attempt to defend their own incompetence. Sadly, this seems to be particularly widespread in Software Engineering, as the intangible, magical thing that is software must therefore cost nothing to create, take no time, cause no difficulty and can be led by people who've never done it. This always leads to disaster.
